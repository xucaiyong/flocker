...
class Losetup(object):
    def add(self, backing_file):
        ...
        return FilePath(result.output.rstrip())

    def remove(self, device_path):
        run_process(
            [b"losetup", b"--detach", device_path.path],
        )


@contextmanager
def temporary_directory(directory_base=None):
    mkdtemp_args = {}
    if directory_base is not None:
        mkdtemp_args["dir"] = directory_base.path
    directory = FilePath(mkdtemp(**mkdtemp_args))
    try:
        yield directory
    finally:
        directory.remove()
...
@contextmanager
def temporary_mount(device_path, mountpoint=None, mountpath_base=None):
    with temporary_directory(
        directory_base=mountpath_base
    ) as temporary_mountpoint:
        if mountpoint is None:
            mountpoint = temporary_mountpoint
        mounted = False
        try:
            run_process(['mount', device_path.path, mountpoint.path])
            mounted = True
            yield mountpoint
        finally:
            if mounted:
